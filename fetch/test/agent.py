...
test = Agent(name="test", seed="test secret", port=8000, endpoint=["http://localhost:8000/submit"])
...
#     await ctx.send(GEMINI_ID, Request(api_key=GEMINI_API_KEY, prompt="Hello, Gemini!"))
...
#     await ctx.send(GEMINI_ID, Request(api_key=GEMINI_API_KEY, chat_history=chat_history))

@test.on_interval(10)
async def on_interval(ctx: Context):
    with open("thor.jpg", "rb") as image_file:
        encoded_string = base64.b64encode(image_file.read())
        await ctx.send(GEMINI_ID, Request(api_key=GEMINI_API_KEY, prompt="Generate a caption for this image", image=Image(format="jpeg", base64=encoded_string)))
